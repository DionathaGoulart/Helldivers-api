...
| `source-labels.json` | `{ <source label>: <source type> }` | Non-warbond acquisition labels (`Starter Equipment` → `default`, `Liberty Day` → `event`). Matched after the same normalization as warbond aliases. An unknown label fails the run, forcing a mapping. |
| `warbond-stubs.json` | `[<warbond id>, …]` | **Temporary.** Warbond ids referenced by the scraped collections (boosters, weapons) until the warbonds collection is scraped; `pnpm validate:data` accepts references to these ids while `warbonds.json` does not exist. |
